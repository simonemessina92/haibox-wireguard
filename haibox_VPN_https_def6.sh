...
# Haibox WireGuard + Port Forwarding Wizard (VPS side)
# Version: haibox_VPN_https_def6.sh
#
# Golden logic:
# - Full tunnel for clients behind GL-AXT1800 via VPS Internet breakout
# - DNAT from public iface and hairpin DNAT from wg0 to reach services using VPS public IPv4
# - StreamHub ports per PDF + fixed TCP/UDP 7900-7940
# - Makito X4E GUI: 10443
# - Makito X4E UDP range: 30000-30004
# - HSG (HMG) GUI: 10444
# - HSG SSH: 2222 -> 22
# - HSG RTMP: 1936 -> 1935
# - HSG SRT UDP range: 9000-9100
# - Optional Proxmox GUI: 8006 -> 8006
#
# NOTE: This script contains an EULA/usage notice displayed ONLY when selecting "1) INSTALL".

STATE_FILE="/root/haibox_wg_state.conf"
APPLIED_STATE_FILE="/root/haibox_wg_applied.conf"
ROUTER_CONF_OUT="/root/haibox_axt1800_wg.conf"
...
SCRIPT_INSTALL_PATH="/usr/local/sbin/haibox_VPN_https_def6.sh"
...
show_eula_if_needed() {
  ...
  cat <<'EOF'
=====================================================================
IMPORTANT NOTICE - INTELLECTUAL PROPERTY AND USAGE TERMS
=====================================================================

This script, including its structure, logic, and implementation,
is the intellectual property of Simone Messina.

This script has been developed as part of a professional technical
project and is provided exclusively for use by Haivision Systems Inc.
for internal activities, commercial demonstrations, proof-of-concept
deployments, and customer-facing use related to Haivision products
and solutions.

Permission to use this script is granted only within Haivision.
Any copying, redistribution, publication, modification, or reuse
of this script, in whole or in part, outside of Haivision is strictly
prohibited without prior written consent from the author.

This script must not be used, adapted, or referenced for third-party
products, competing solutions, or non-Haivision commercial activities.

This script is provided "as is", without warranty of any kind.
The author assumes no responsibility or liability for damages,
misconfigurations, service interruptions, or data loss resulting
...
print_config() {
  ...
  echo "  GL-AXT1800 WG IP:        ${WG_GL_IP}"
  echo
...
prompt_config() {
  ...
  read -r -p "GL-AXT1800 WG IP [${WG_GL_IP}]: " v; [[ -n "${v}" ]] && WG_GL_IP="${v}"
...
ROUTER_CONF_OUT = "/root/haibox_axt1800_wg.conf"
REMOTE_CLIENT_CONF_OUT = "/root/haibox_remote_client_wg.conf"
SCRIPT_PATH = "/usr/local/sbin/haibox_VPN_https_def6.sh"
WEBUI_SERVICE_NAME = "haibox-webui.service"
          ...
          <form method="post" action="/apply">
            ...
            <section class="config-block">
              ...
              <div class="grid">
                ...
                {input_row("GL-AXT1800 WG IP", "WG_GL_IP", state.get("WG_GL_IP", ""))}
              </div>
            </section>
...
class Handler(BaseHTTPRequestHandler):
    server_version = "HaiboxWebUI/6.0"
